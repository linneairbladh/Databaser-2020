			...
			public void actionPerformed(ActionEvent e) {
				...
				if (!ssn.isEmpty()) {
					try {
						...
						if (s == null) {
							frame.getTextArea_Student()
									.setText("Unable to find a student with the given SSN, check spelling ETC");
						} else {
					...
					try {
						...
						if (student == null || course == null) {
							...
							if (hasstudiedlist.isEmpty()) {
								frame.getTextArea_Student().setText(
										"Student with SSN: " + student.getSsn() + " has not examined from the course");
							} else {
								String sr = "";
			...
			public void actionPerformed(ActionEvent e) {
				...
				if (courseCode.isEmpty()) {
					...
					try {
						...
						if (c != null) {
							frame.getTextArea_Course()
									.setText("The following Course found; " + "\nCourseCode: " + " " + c.getCourseCode()
						...
						if (course == null) {
							...
							if (courseResultList.isEmpty()) {
								...
								for (HasStudied hs : courseResultList) {
									...
									frame.getTextArea_Course().append(
											"Student: " + hs.getStudentSsn() + " grade: " + hs.getGrade() + "\n");
				...
				if (courseCode.isEmpty() || ssn.isEmpty()) {
					...
					try {
						...
						if (student == null) {
							...
							frame.getTextArea_Register()
									.setText("StudentName: " + student.getStudentName() + "\nStudentSSN: "
											+ student.getSsn() + "\nWas registred on course: "
				...
				if (courseCode.isEmpty() || ssn.isEmpty() || grade.isBlank()) {
					...
					try {
						...
						if (student == null) {
							...
							frame.getTextArea_Register()
									.setText("The result [" + grade + "] for following student was added; " + "\nSSN: "
